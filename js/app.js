...
async function loadPremiumHero() {
    ...
    try {
        ...
        document.getElementById('hero-whatsapp').href = `https://wa.me/34910922098?text=Hola,%20estoy%20interesado%20en%20el%20${encodeURIComponent(premiumCar.brand + ' ' + premiumCar.model)}%20de%20Autos%20Sanchez%20Guerrero.`;

        // Calculate discount dynamically (~10% above current price, rounded to 500)
        const originalPrice = Math.round(premiumCar.price * 1.10 / 500) * 500;
...
async function initCounters() {
    ...
    // Try to update counter targets from inventory data
    try {
        const response = await fetch('data/inventory.json');
        const data = await response.json();
        const sellCount = data.sellCount || data.cars.filter(c => c.sold).length;
        if (sellCount > 0) {
            counters.forEach(el => {
                if (!el.dataset.target || el.dataset.target === '35') {
                    el.dataset.target = sellCount;
                }
            });
        }
    } catch (_) { /* use html defaults */ }
