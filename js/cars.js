...
async function fetchInventory() {
    try {
        ...
        // Set max price from inventory data
        const maxPrice = Math.max(...allCars.map(c => c.price), 30000);
        ...
        if (priceVal) {
            priceVal.textContent = new Intl.NumberFormat('es-ES').format(roundedMax) + '€';
        }
...
function initFilters() {
    ...
    if (resetBtn) resetBtn.addEventListener('click', () => {
        ...
        if (priceRange) {
            const maxVal = parseInt(priceRange.max) || 30000;
            filters.maxPrice = maxVal;
            priceRange.value = maxVal;
            priceVal.textContent = new Intl.NumberFormat('es-ES').format(maxVal) + '€';
        }
